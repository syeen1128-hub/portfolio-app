...
function scoreNews(title) {
  ...
  for (const w of goodWords) {
    if (t.includes(w.toLowerCase())) good += 1;
  }

  for (const w of badWords) {
    if (t.includes(w.toLowerCase())) bad += 1;
  }
...
async function getNewsResult(item) {
  ...
  if (!items.length) {
    return fallbackResult(item, "no-news");
  }
