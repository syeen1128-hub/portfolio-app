...
async function quoteKr(item) {
  ...
  try {
    ...
    const price = cleanNumber(
      data?.nv || data?.closePrice || data?.nowVal
    );
  ...
  try {
    const j = await fetchJson(
      `https://api.finance.naver.com/service/itemSummary.naver?itemcode=${encodeURIComponent(code)}`
    );
  ...
  try {
    const txt = await fetchText(
      `https://finance.naver.com/item/sise.naver?code=${encodeURIComponent(code)}`
    );
  ...
  try {
    const j = await fetchJson(
      `https://finance.daum.net/api/quotes/A${encodeURIComponent(code)}?summary=false`,
      {
