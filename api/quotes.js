...
function withTimeout(promise, ms, item) {
  return Promise.race([
    ...
    new Promise(resolve =>
      setTimeout(
        () => resolve({ id: item.id, price: null, time: nowStamp(), source: "timeout" }),
        ms
      )
    )
  ]);
...
async function quoteYahoo(item) {
  ...
  try {
    const j = await fetchJson(`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?range=5d&interval=1d`, {
      referer: "https://finance.yahoo.com/"
    });
